(function() {
    'use strict';
    const d = document;
    const originalLoc = d.location.href; 
    ...
    const css = `
        ...
        :root { --primary: #5e9cff; --primary-dark: #4b7dcc; --primary-glow: rgba(94, 156, 255, 0.5); --bg: #0a0a0a; --card: rgba(20, 20, 22, 0.85); --border: rgba(255, 255, 255, 0.1); --text: #e0e0e0; --text-muted: #999; --radius: 16px; --anim: 0.3s cubic-bezier(0.2, 0.8, 0.2, 1); }
        body { margin: 0; overflow: hidden; background: var(--bg); font-family: 'Cairo', sans-serif; color: var(--text); }
        #qm-root { display: flex; height: 100vh; width: 100vw; overflow: hidden; direction: rtl; opacity: 0; animation: fadeIn 0.5s var(--anim) forwards; }
        ...
        #qm-sidebar { width: 400px; min-width: 400px; background: var(--card); border-left: 1px solid var(--border); display: flex; flex-direction: column; padding: 24px; box-sizing: border-box; box-shadow: -10px 0 40px rgba(0,0,0,0.5); z-index: 1000; backdrop-filter: blur(20px); position: relative; }
        ...
        @media (max-width: 768px) { #qm-root { flex-direction: column-reverse; } #qm-sidebar { width: 100%; min-width: 100%; height: 65vh; border-left: none; border-top: 1px solid var(--border); border-radius: 24px 24px 0 0; padding: 20px; } #qm-frame-box { margin: 0; border-radius: 0; } }
        .qm-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; }
        .qm-title { margin: 0; font-family: 'IBM Plex Sans Arabic', sans-serif; font-size: 1.8rem; font-weight: 700; background: linear-gradient(90deg, var(--primary-dark), var(--primary)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-shadow: 0 0 30px rgba(94, 156, 255, 0.3); }
        .qm-close { background: rgba(255,255,255,0.05); color: var(--text-muted); border: 1px solid var(--border); width: 40px; height: 40px; border-radius: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; transition: var(--anim); padding: 0; line-height: 0; }
        .qm-close:hover { background: rgba(255, 82, 82, 0.2); color: #ff5252; border-color: rgba(255, 82, 82, 0.3); transform: rotate(90deg); }
        .qm-stats { display: flex; justify-content: space-between; align-items: center; font-size: 0.9rem; color: var(--text-muted); margin-bottom: 15px; font-weight: 600; padding: 0 5px; }
        ...
        .qm-card-ui { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; background: rgba(255,255,255,0.03); border: 1px solid var(--border); border-radius: var(--radius); transition: var(--anim); color: var(--text); position: relative; overflow: hidden; }
        .qm-card-ui span { font-size: 1rem; font-weight: 600; z-index: 2; }
        .qm-card-ui::before { content: ''; position: absolute; inset: 0; background: radial-gradient(circle at 100% 0, rgba(94, 156, 255, 0.1) 0%, transparent 60%); opacity: 0; transition: var(--anim); z-index: 1; }
        ...
        .qm-item input:checked ~ .qm-card-ui { border-color: var(--primary); background: rgba(94, 156, 255, 0.08); box-shadow: 0 0 20px rgba(94, 156, 255, 0.15), inset 0 0 0 1px rgba(94, 156, 255, 0.1); }
        .qm-item input:checked ~ .qm-card-ui span { color: #fff; }
        .qm-icon { width: 24px; height: 24px; border-radius: 50%; border: 2px solid rgba(255,255,255,0.2); display: flex; align-items: center; justify-content: center; transition: var(--anim); color: transparent; font-size: 14px; z-index: 2; }
        .qm-item input:checked ~ .qm-card-ui .qm-icon { background: var(--primary); border-color: var(--primary); color: #fff; box-shadow: 0 0 15px var(--primary-glow); }
        .qm-controls { display: flex; flex-direction: column; gap: 15px; margin-top: auto; }
        .custom-select { position: relative; width: 100%; }
        .select-trigger { display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; background: rgba(0,0,0,0.3); color: var(--text); border: 1px solid var(--border); border-radius: var(--radius); font-size: 1rem; font-weight: 600; cursor: pointer; transition: var(--anim); user-select: none; }
        .select-trigger:hover, .custom-select.active .select-trigger { border-color: var(--primary); background: rgba(0,0,0,0.5); color: #fff; }
        .select-trigger::after { content: ''; border: 6px solid transparent; border-top-color: var(--text-muted); margin-top: 4px; transition: var(--anim); }
        .custom-select.active .select-trigger::after { transform: rotate(180deg); border-top-color: var(--primary); margin-top: -4px; }
        .select-options { position: absolute; bottom: 110%; left: 0; right: 0; background: rgba(25, 25, 30, 0.95); border: 1px solid var(--border); border-radius: var(--radius); overflow: hidden; opacity: 0; visibility: hidden; transform: translateY(10px); transition: var(--anim); z-index: 100; backdrop-filter: blur(20px); box-shadow: 0 10px 40px rgba(0,0,0,0.5); }
        .custom-select.active .select-options { opacity: 1; visibility: visible; transform: translateY(0); }
        .option { padding: 14px 20px; cursor: pointer; color: var(--text); transition: var(--anim); border-bottom: 1px solid rgba(255,255,255,0.05); font-weight: 600; }
        ...
        #qm-run { width: 100%; padding: 18px; border: none; border-radius: 99px; background: linear-gradient(90deg, var(--primary-dark), var(--primary)); color: #fff; font-family: 'IBM Plex Sans Arabic', sans-serif; font-size: 1.2rem; font-weight: 700; cursor: pointer; box-shadow: 0 4px 20px rgba(94, 156, 255, 0.3); transition: var(--anim); display: flex; justify-content: center; align-items: center; gap: 10px; }
        ...
        #qm-status { margin-top: 20px; text-align: center; font-size: 1rem; font-weight: 600; color: #ffc400; min-height: 24px; transition: var(--anim); }
        .qm-footer { margin-top: 20px; text-align: center; font-size: 0.85rem; color: var(--text-muted); padding-top: 15px; border-top: 1px solid var(--border); font-family: 'IBM Plex Sans Arabic', sans-serif; }
    ...
    <div id="qm-root">
        <div id="qm-sidebar">
            <div class="qm-header">
                <h2 class="qm-title">المقيم الآلي (مطور)</h2>
        ...
        <div id="qm-frame-box"><iframe id="qm-ifr" src="${originalLoc}"></iframe></div>
    </div>`;
    ...
    let queue = [], active = false, retries = 0, noTableRetries = 0;

    const updateUI = () => {
        const n = d.querySelectorAll('.chk:checked').length;
        cnt.innerText = `${n}/${courses.length}`;
        btn.innerHTML = n > 0 ? `<span>بدء تقييم (${n})</span> 🚀` : 'يرجى اختيار مواد';
        btn.disabled = n === 0;
        if(n > 0) btn.classList.add('pulse'); else btn.classList.remove('pulse');
    };
    ...
    const processQueue = () => {
        ...
        const timer = setInterval(() => {
            try {
                ...
                const bodyTxt = doc.body.innerText;
                const hasMsg = doc.getElementById('frm:errorMsg2');
                const hasCheck = bodyTxt.includes('تم حفظ') || bodyTxt.includes('Saved') || bodyTxt.includes('Completed');
                
                if (hasMsg || hasCheck) {
                    st.innerText = '🔙 تم الحفظ، العودة للقائمة...';
                ...
                if (doc.querySelector('table.rowFlow')) {
                    ...
                    if (link) {
                        ...
                        retries++;
                        st.innerText = `...جار البحث عن المادة (${retries})`;
                        if (retries > 40) { 
                            st.innerText = `⚠️ تخطي ${cid} (لم تظهر)`;
                            queue.shift(); 
                            retries = 0; 
                        }
                    }
                ...
                if (radios.length) {
                    ...
                    doc.querySelectorAll('table tbody tr').forEach(row => {
                        ...
                        if (rds.length > 2) {
                            ...
                            if (/ظلل|تأكد|select|خيار|choose|consistent|agree|موافق/.test(txt) && /بشدة|strongly/.test(txt) === false && /please|يرجى/.test(txt)) {
                    ...
                    script.textContent = "if(typeof submitForm=='function'){submitForm('/qu')}else if(document.forms.length > 0){document.forms[0].submit()}";
                ...
                noTableRetries++;
                if(noTableRetries > 5) {
                    st.innerText = '⚠️ الصفحة غير صحيحة، جاري التصحيح...';
                    ifr.src = originalLoc;
                    noTableRetries = 0;
                }
